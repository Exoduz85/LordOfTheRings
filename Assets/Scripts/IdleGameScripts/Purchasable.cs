...
public class Purchasable{
    ...
    Resource gold;
    string productId;
    bool IsAffordable => this.gold.ResourceAmount >= this.goldProductionData.GetActualCosts(this.Amount);
    public int Amount {
    ...
    public void Purchase() {
        if (!this.IsAffordable) 
            return;
        this.gold.ResourceAmount -= this.goldProductionData.GetActualCosts(this.Amount);
        this.Amount += 1;
